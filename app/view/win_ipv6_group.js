...
Ext.define('NFW2.view.win_ipv6_group', {
    extend: 'Ext.window.Window',
    alias: 'widget.win_ipv6_group',

    requires: [
        'NFW2.view.win_ipv6_groupViewModel',
        'Ext.form.Panel',
        'Ext.form.field.Text',
        'Ext.grid.Panel',
        'Ext.view.Table',
    ...
    listeners: {
        ...
        close: 'onWin_ipv6_groupClose',
        resize: 'onWin_ipv6_groupResize'
    },
                                ...
                                columns: [
                                    ...
                                    },
                                    {
                                        ...
                                        items: [
                                            {
                                                handler: function(view, rowIndex, colIndex, item, e, record, row) {
                                                    var me = Ext.getCmp("win_ipv6_group");
                                                    ...
                                                    if(tmptd[1]){
                                                        ...
                                                        Ext.getCmp("win_ipv6_group").get_obj_info(cid,otype,"grid_group_setobj",rowIndex,(record.data.xname===undefined)?record.data.name:record.data.xname,_sel);
                                                    }
                                                },
                                                getClass: function(v, metadata, r, rowIndex, colIndex, store) {
                                            ...
                                            },
                                            {
                                                handler: function(view, rowIndex, colIndex, item, e, record, row) {
                                                    var otype = record.data.otype;
                                                    var loadmode = "store_ipv6_group_obj";
                                                    var target = "spd_ip";
    ...
    onButtonClick2: function(button, e, eOpts) {
        ...
        var _count = (me.loadmode===undefined)?Ext.getCmp("ipv6Group").count:null;
        ...
        if(me.name !== name.getValue()){
            ...
            request_helper.xmlrpc_call_JsonP(
                ...
                function(response){
                    ...
                    if(response !== null){
                        prt_errMsg(get_msg('err_namedob'),'fld_msg2'); name.focus(); return false;
                    }else{
                        fn_set();
                    }
                }
                ...
                success : function(response){
                    ...
                    if(response.retcode === true){
                        ...
                        if(me.loadmode === undefined){
                            var _store = Ext.data.StoreManager.lookup("store_object_ipv6_group_list");
                        }else{

                            if(me.target !== undefined){
                                var __cid = (update)?me.cid:response.retval;
                                addFWItem(me.target, update, name.getValue(), "v6_group", __cid, m_count);
                            }
                        ...
                        _store.load(function(records, options, success) {
                            if(me.loadmode === undefined && !me.edit){
                                ...
                                Ext.getCmp("disp_ipv6_group_total").setValue(tot.total+'/'+tot.max_count);
                            }
                        });
    ...
    onTextfieldKeydown: function(textfield, e, eOpts) {
        ...
        var _store = "store_ipv6_group_obj";
    ...
    onWin_ipv6_groupClose: function(panel, eOpts) {
        Ext.data.StoreManager.lookup("store_tmp_group").removeAll();
    },

    onWin_ipv6_groupResize: function(window, width, height, eOpts) {
        var _hei1 = height-80;
    ...
    make_inlist: function(mode) {
        ...
        Ext.getCmp('grid_group_setobj').reconfigure("store_ipv6_group_obj");
        Ext.data.StoreManager.lookup('store_ipv6_group_obj').getProxy().setExtraParam("object_list",  Ext.encode(["ipv6"]));
        if(init){
            Ext.data.StoreManager.lookup('store_ipv6_group_obj').getProxy().setExtraParam("search_info",  Ext.encode([]));
        }
        Ext.data.StoreManager.lookup('store_ipv6_group_obj').load();
    },
