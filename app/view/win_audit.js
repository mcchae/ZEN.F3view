...
Ext.define('NFW2.view.win_audit', {
    extend: 'Ext.window.Window',
    alias: 'widget.win_audit',

    requires: [
        'NFW2.view.win_auditViewModel',
        'Ext.form.field.Date',
        'Ext.form.Label',
    ...
    make_list: function(obj_x, uid) {
        ...
        var btn_dv = "<table width='100%'><tr><td width='10%'></td><td width='25%'><input type='button' value='"+__zen('add')+"' onclick=\"addAuditRule('sip','v4')\" /></td><td width='25%'><input type='button' value='"+__zen('add')+"' onclick=\"addAuditRule('dip','v4')\" /></td><td width='40%'><input type='button' value='"+__zen('add')+"'  onclick=\"addAuditRule('svc','v4')\"/></td></tr></table>";
        ...
        for (var k=0; k<obj.sip.length; k++) {
            tmp_cls = (k===0)?"selon":"";
            sip_li += "<li class='"+tmp_cls+"' onClick=\"get_audit_filter('"+obj.sip[k]+"','sip',this)\">"+obj.sip[k]+"</li>";
        }

        for (var k=0; k<obj.dip.length; k++) {
            tmp_cls = (k===0)?"selon":"";
            dip_li += "<li class='"+tmp_cls+"' onClick=\"get_audit_filter('"+obj.dip[k]+"','dip',this)\">"+obj.dip[k]+"</li>";
        }

        for (var k=0; k<obj.service.length; k++) {
            tmp_cls = (k===0)?"selon":"";
            svc_li += "<li class='"+tmp_cls+"' onClick=\"get_audit_filter('"+obj.service[k]+"','svc',this)\">"+obj.service[k]+"</li>";
        }
        ...
        get_audit_filter(sip_tmp,'sip',null);
        get_audit_filter(dip_tmp,'dip',null);
        get_audit_filter(svc_tmp,'svc',null);
