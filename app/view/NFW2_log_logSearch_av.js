...
Ext.define('NFW2.view.NFW2_log_logSearch_av', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nfw2_log_logsearch_av',

    requires: [
        'NFW2.view.NFW2_log_logSearch_avViewModel',
        'Ext.form.Panel',
        'Ext.form.Label',
        'Ext.button.Button',
        'Ext.form.field.Date',
    ...
    onButtonClick: function(button, e, eOpts) {
        ...
        var store = Ext.data.StoreManager.lookup('store_log_av');
        ...
        for(var i=0; i<chk.length; i++){
            if(chk[i].checked){
                _orSearch.push({ 'event': parseInt('0x'+chk[i].value) });
            }
        }
        ...
        _search_params['schema_name'] = Ext.encode('AV');
    ...
    onPanelAfterRender: function(component, eOpts) {
        ...
        var _params = {
            ...
            cond: Ext.encode({'category':'안티바이러스'})
        };
            ...
            function(response){
                ...
                for(var i=0; i<a_list.length; i++){
                    ...
                    for(var l=0; l<record.length; l++){
                        ...
                        list += '<td width="50%"><label><input type="checkbox" name="chk[]" value="'+record[l].event+'" class="code_'+_a_list[i]+'" onclick="chk_log_event(this)" />'+record[l].text+'</label></td>';

                        if((l+1)%2 === 0 || l === sub_len){ if((l+1)%2===2){ list+='<td width="50%"></td>'; }list += '</tr>'; }
                        if(l === sub_len){ list += '</table>'; }
                        s_event.push(list);
                    }
                    var t_event = '<tr><td style="border:1px solid #ccc;background:#f5f5f7"><label><input type="checkbox" id="all_'+_a_list[i]+'" class="code_'+_a_list[i]+'" onclick="chk_log_event(this)" />'+record[0].category+'</label></td>'+
    ...
    tag_validate: function(value) {
        ...
        }else if(val[0] !== __zen('action').toLowerCase() && val[0] !== __zen('desc').toLowerCase() && val[0] !== __zen('summary').toLowerCase()){
            return false;
        }
        Ext.getCmp("err_msg").hide();
        return true;
    ...
    query_detail: function(type, val) {
        ...
        var text = (type==='protocol')?__zen('protocol'):(type==='action')?__zen('action'):(type==="desc")?__zen('desc'):(type==='summary')?__zen('summary'):"";
    ...
    log_celldbclick: function(cellIndex, record) {
        ...
        if(show_mode === 1){
            if(cellIndex === 3){
                ...
                var value = record.data.action;
                var act = (value===0)?__zen('block'):(value===1)?__zen('allow'):(value===2)?"IPSec":"";
                _val = __zen('action')+"="+act;
