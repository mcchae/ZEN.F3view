...
Ext.define('NFW2.view.NFW2_log_logSearch_system', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nfw2_log_logsearch_system',

    requires: [
        'NFW2.view.NFW2_log_logSearch_systemViewModel',
        'Ext.form.Panel',
        'Ext.form.Label',
        'Ext.button.Button',
        'Ext.form.field.Date',
    ...
    viewModel: {
        type: 'nfw2_log_logsearch_system'
    },
    cls: 'NFW2_log',
    id: 'NFW2_log',
    minHeight: 300,
            ...
            items : [
                {
                    ...
                    '<div class="list" onclick=log_help(this)>'+__zen('user')+'</div>'+
    ...
    onButtonClick: function(button, e, eOpts) {
        ...
        var store = Ext.data.StoreManager.lookup('store_log_system');
        ...
        _search_params['schema_name'] = Ext.encode('SYSTEM');
        ...
        if(Ext.getCmp("dis_mode").getValue() === "display"){
            ...
            var fileName = Ext.Date.format(new Date(), 'Ymd')+"_SYSTEM.xlsx";
            var path = '/mnt/ramdisk/do/F3view/nodeapps/deploy/public/javascripts/attachments/';

            var _condition = {
                schema_name: Ext.encode('SYSTEM'),
    ...
    onPanelAfterRender: function(component, eOpts) {
        ...
        request_helper.xmlrpc_call_JsonP(
            ...
            function(response){
                ...
                var r_event = [];
                var a_list = ['시스템','관리자','정책 전송','데몬 관리','백업/복원','업데이트','로그','외부서버','설정','인증서','센터관리','HA','LLCF'];
                var _a_list = ['sys','admin','send','daemon','backup','update','log','radius','set','cert','center','ha','llcf'];
    ...
    tag_validate: function(value) {
        ...
        }else if(val[0] !== __zen('user').toLowerCase() && val[0] !== __zen('desc').toLowerCase() && val[0] !== __zen('summary').toLowerCase()){
            return false;
        }
        Ext.getCmp("err_msg").hide();
        return true;
    ...
    query_detail: function(type, val) {
        ...
        var text = (type==='protocol')?__zen('protocol'):(type==='user')?__zen('user'):(type==="desc")?__zen('desc'):(type==='summary')?__zen('summary'):"";
    ...
    log_celldbclick: function(cellIndex, record) {
        ...
        if(show_mode === 1){
            if(cellIndex === 3){
                ...
                _val = __zen('user')+"="+record.data.user;
