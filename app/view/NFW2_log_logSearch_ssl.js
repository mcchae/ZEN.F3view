...
Ext.define('NFW2.view.NFW2_log_logSearch_ssl', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nfw2_log_logsearch_ssl',

    requires: [
        'NFW2.view.NFW2_log_logSearch_sslViewModel',
        'Ext.form.Panel',
        'Ext.form.Label',
        'Ext.button.Button',
        'Ext.form.field.Date',
            ...
            items : [
                {
                    ...
                    '<div class="list" onclick=log_help(this)>'+__zen('login_id')+'</div>'+
    ...
    onButtonClick: function(button, e, eOpts) {
        ...
        var store = Ext.data.StoreManager.lookup('store_log_ssl');
        ...
        _search_params['schema_name'] = Ext.encode('SSL');
        ...
        if(Ext.getCmp("dis_mode").getValue()==="display"){
            ...
            var fileName = Ext.Date.format(new Date(), 'Ymd')+"_SSL.xlsx";
            var path = '/mnt/ramdisk/do/F3view/nodeapps/deploy/public/javascripts/attachments/';

            var _condition = {
                schema_name: Ext.encode('SSL'),
    ...
    onPanelAfterRender: function(component, eOpts) {
        ...
        var _params = {
            ...
            cond: Ext.encode({'category':'SSLVPN'})
        };
        ...
        request_helper.xmlrpc_call_JsonP(
            ...
            function(response){
                ...
                var r_event = [];
                var a_list = ["사용자 인증","액세스","트래픽","무결성 검사"];
                var _a_list = ['user','access','flow','inter'];
                ...
                for(var i=0; i<a_list.length; i++){
                    ...
                    var t_event = '<tr><td width="30%" style="border:1px solid #ccc;background:#f5f5f7"><label><input type="checkbox" id="all_'+_a_list[i]+'" class="code_'+_a_list[i]+'" onclick="chk_log_event(this)" />'+record[0].category+'</label></td>'+
        ...
        var awe = new Awesomplete(input, {
            list: ['출발지','출발지 포트','목적지','목적지 포트','프로토콜','정보','로그인 계정','프로토콜=IP','프로토콜=ICMP',
    ...
    tag_validate: function(value) {
        ...
        }else if(val[0] !== __zen('login_id').toLowerCase() && val[0] !== __zen('desc').toLowerCase() && val[0] !== __zen('summary').toLowerCase()){
            return false;
        }
        Ext.getCmp("err_msg").hide();
        return true;
    ...
    query_detail: function(type, val) {
        ...
        var text = (type==='protocol')?__zen('protocol'):(type==='user')?__zen('login_id'):(type==="desc")?__zen('desc'):(type==="summary")?__zen('summary'):"";
    ...
    log_celldbclick: function(cellIndex, record) {
        ...
        and.className = 'nsg-tag';
        and.innerHTML = 'and';

        var node = document.createElement("span");
        node.className = 'nsg-tag';
        ...
        if(show_mode === 1){
            if(cellIndex === 3){
                ...
                if(sport !== 0 && record.data.protocol !== 1000){
                ...
                if(dport !== 0 && record.data.protocol !== 1000){
                    $("#tagedit-input").val(__zen('dest_port')+"="+dport);
                    $("#tagedit-input").trigger('transformToTag');
                }
                return false;
            }else if(cellIndex === 9){
                if(record.data.protocol===0 || record.data.protocol===1000){ return false; }
                _val = __zen('protocol')+"="+getProtocol(record.data.protocol);
            }else if(cellIndex === 15){
                _val = __zen('login_id')+"="+record.data.user;
