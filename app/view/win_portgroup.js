...
Ext.define('NFW2.view.win_portgroup', {
    extend: 'Ext.window.Window',
    alias: 'widget.win_portgroup',

    requires: [
        'NFW2.view.win_portgroupViewModel',
        'Ext.form.Panel',
        'Ext.grid.Panel',
        'Ext.view.Table',
                                ...
                                columns: [
                                    ...
                                    },
                                    {
                                        ...
                                        items: [
                                            {
                                                handler: function(view, rowIndex, colIndex, item, e, record, row) {
                                                    var me = Ext.getCmp("win_portgroup1");
                                                    ...
                                                    if(tmptd[1]){
                                                        ...
                                                        Ext.getCmp("win_portgroup1").get_obj_info(cid,otype,"grid_group_setobj",rowIndex,(record.data.xname===undefined)?record.data.name:record.data.xname,_sel);
                                                    }
                                                },
                                                getClass: function(v, metadata, r, rowIndex, colIndex, store) {
    ...
    processMyCheckItem1: function(config) {
        config.text = __zen('protocol');
        return config;
    },

    processMyCheckItem2: function(config) {
        config.text = __zen('port');
        return config;
    },
    ...
    onButtonClick2: function(button, e, eOpts) {
        ...
        var _count = (me.loadmode===undefined)?Ext.getCmp("serviceGroup").count:null;
        ...
        var _params = {
            object_service_group : Ext.encode(obj),
                ...
                function(response){
                    ...
                    if(me.loadmode === undefined){
                        ...
                        if(me.target !== undefined){
                            ...
                            addFWItem(me.target, update, name.getValue(), "group", __cid, m_count);
                        }
                    ...
                    if(me.loadmode===undefined && _store.getTotalCount()+1 >= _count){
                        me.close();
                        return false;
                    }
    ...
    onWindowAfterRender: function(component, eOpts) {
        ...
        var _params = {
            filename: Ext.encode('/proc/ferret/datasheet/service_grp_obj_member')
        };
        ...
        if(me.edit === "edit"){
            ...
            me.setTitle(__zen('service_group_edit')+" - "+me.num);
            ...
            me.setTitle(__zen('service_group_add'));
        }
        ...
        Ext.getCmp("search_pro").bindStore(p_store);
    ...
    onSear_typeChange: function(cycle, item, eOpts) {
        if(item.itemIndex === 1){
            Ext.getCmp("search_val").hide();
            Ext.getCmp("search_pro").show();
        }else{
            Ext.getCmp("search_val").show();
            Ext.getCmp("search_pro").hide();
        }
    },
    ...
    onTextfieldKeydown: function(textfield, e, eOpts) {
        ...
        switch (settype){
            ...
            case 2	: var _params = {type:'port',port:Number(textfield.value)}; break;
        }
    ...
    onSearch_proChange: function(field, newValue, oldValue, eOpts) {
        var settype = Ext.ComponentQuery.query('#sear_type')[0].value;
        var _store = "store_ip_group_obj";

        switch (settype){
            case 1 	: var _params = {type:'port',protocol:newValue}; break;
        }
    ...
    onWin_portgroup1Resize: function(window, width, height, eOpts) {
        var _hei1 = height-80;
    ...
    get_obj_info: function(cid, otype, grid, rowIndex, name, _sel) {
        ...
        var _params = {
            basename: Ext.encode("with_cid"),
            key: Ext.encode({ '_kind': 'object_service_port','@cid': cid })
        };
    ...
    make_inlist: function(mode) {
        ...
        Ext.data.StoreManager.lookup('store_ip_group_obj').getProxy().setExtraParam("object_list",  Ext.encode(["port"]));
        if(init){
