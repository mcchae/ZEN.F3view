...
Ext.define('NFW2.view.monitor_ipm_allowhost_con', {
    extend: 'Ext.window.Window',
    alias: 'widget.monitor_ipm_allowhost_con',

    requires: [
        'NFW2.view.monitor_ipm_allowhost_conViewModel',
        'Ext.form.Panel',
        'Ext.form.Label',
    ...
    viewModel: {
        type: 'monitor_ipm_allowhost_con'
    },
    cls: 'zen_win',
    id: 'monitor_ipm_allowhost_con',
    defaultListenerScope: true,

    items: [
        {
            xtype: 'form',
            itemId: 'ipm_allowhost_fm',
            bodyPadding: 10,
            header: false,
            title: 'My Form',
            items: [
            ...
            request_helper.xmlrpc_call_JsonP(
                ...
                function(response){
                    ...
                    request_helper.xmlrpc_call_JsonP(
                        ...
                        function(response){
                            Ext.getCmp('monitor_ipm_host_con').disable(true);
                            Ext.getCmp('toolbar').disable(true);
                            Ext.getCmp('NFW2_monitor_ipm_map').get_ipm_map();
                            Ext.Msg.show({
                                title: __weguardia,
                                width: 300,
    ...
    onButtonClick7: function(button, e, eOpts) {
        var me = Ext.getCmp('monitor_ipm_allowhost_con');
        ...
        request_helper.xmlrpc_call_JsonP(
            ...
            function(data){
                ...
                for(var i in data.list){
                    var ip_list = [];
                    var obj = [];
                    var store = Ext.getCmp('ipm_host_ip_grid').getStore();
                    ...
                    for(var k in ip_list){
                        for(var l in store.data.items){
                            if(store.data.items[l].data.ip === ip_list[k]){
                                obj = data.list[i];
                            }
                        }
                    }
                ...
                if(response.retcode){
                    ...
                    request_helper.xmlrpc_call_JsonP(
                        ...
                        function(data){
                            ...
                            console.log(me);
                            if(me.set_value !== undefined){
                                var value = me.set_value;
                                console.log(value);
                                Ext.getCmp('monitor_ipm_host_con').enable(true);
                                Ext.getCmp('toolbar').enable(true);
                                var ip_record = [];
                                var mac_record = [];

                                ip_record.push({'ip':value.ip});
                                ip_record.push({'ip':''});
                                ip_record.push({'ip':''});
                                ip_record.push({'ip':''});
                                mac_record.push({'mac':value.mac});
                                mac_record.push({'mac':''});
                                mac_record.push({'mac':''});
                                mac_record.push({'mac':''});
                                ...
                                if(value.action === "on_accept"){ Ext.getCmp('ipm_map_del').show(); }
                                else{ Ext.getCmp('ipm_map_del').hide(); }

                                var store = Ext.data.StoreManager.lookup('store_ipm_host_manager_list');
                                Ext.getCmp('monitor_ipm_allowhost_con').record = me.chk_network;
                                for(var i in store.data.items){
                                    if(store.data.items[i].data.ip === me.chk_network.network){
                                        Ext.getCmp('ipm_host_inter').setValue(store.data.items[i].data.name);
                                    }
                                }
                            }
                            else{
                                if(Ext.getCmp('map_table_con').chk_network !== undefined){
                                    Ext.getCmp('monitor_ipm_host_con').enable(true);
                                    Ext.getCmp('toolbar').enable(true);
                                    Ext.getCmp('ipm_map_del').hide();
                                    var temp = Ext.getCmp('map_table_con').chk_network.data.network.split('.');
                                    var ip_record = [];
                                    var mac_record = [];

                                    ip_record.push({'ip':temp[0]+"."+temp[1]+"."+temp[2]+"."+me.box_num});
                                    ip_record.push({'ip':''});
                                    ip_record.push({'ip':''});
                                    ip_record.push({'ip':''});
                                    mac_record.push({'mac':''});
