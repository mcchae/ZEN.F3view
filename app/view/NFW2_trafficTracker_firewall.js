...
Ext.define('NFW2.view.NFW2_trafficTracker_firewall', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nfw2_traffictracker_firewall',

    requires: [
        'NFW2.view.NFW2_trafficTracker_firewallViewModel',
        'Ext.form.Panel',
        'Ext.form.Label',
        'Ext.button.Button',
        'Ext.form.field.Date',
    ...
    viewModel: {
        type: 'nfw2_traffictracker_firewall'
    },
    cls: 'zen_body',
    id: 'NFW2_tracker',
                                    ...
                                    items: [
                                        {
                                            ...
                                            editable: false,
                                            displayField: 'name',
                                            store: 'store_tracker_fir_action',
                                            valueField: 'val',
                                            bind: {
                                                fieldLabel: '{action}'
                                            }
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            renderer: function(value, metaData, record, rowIndex, colIndex, store, view) {
                                ...
                                if(value !== 4){
                                    return '<img src="../images/icon_op'+value+'.png"/>';
                                }else{
                                    return "PAN";
                                }
                            },
                        ...
                        },
                        {
                            ...
                            renderer: function(value, metaData, record, rowIndex, colIndex, store, view) {
                                ...
                                var text = (type==='sip')?record.data.sip:(type==='dip')?record.data.dip:(type==='protocol')?record.data.protocol:(type==='source_country')?record.data.source_country:(type==='dest_country')?record.data.dest_country:(type==='spd_id')?record.data.spd_id:'';
                                ...
                                var spd_id = (column.spd_id)?'':'<button class="dbtn uid" title="'+__zen('policy')+'" onclick="tracker_depth(\'spd_id\',\''+val+'\',true)" />';
                                ...
                                if(depth.dep_0.type === "date" && !depth.dep_0.text){ h_step = 6; }

                                if(me.purpose === "1" && depth.step < h_step){
                                    return sip+dip+protocol+source_country+dest_country;
                                }else{
                                    return sip+dip+protocol+spd_id+source_country+dest_country+time;
                                }
                            },
                            id: 'c_opt',
                            align: 'center',
    ...
    onS_purposeChange: function(field, newValue, oldValue, eOpts) {
        ...
        if(newValue === '1'){
            Ext.getCmp("traffic_"+me.a_id[0]).setValue('sip');
            _store.removeAt(3,1);
        }
    ...
    onImageAfterRender1: function(component, eOpts) {
        ...
        component.getEl().on('click', function(eOpts) {
            ...
            var a_traffic = ["traffic","ip","protocol","port","id","c_scode","c_dcode"];
            var record = {
                'sip':{
                    name: __zen('src'),
                    val: 'sip'
    ...
    onTraffic_1Change: function(field, newValue, oldValue, eOpts) {
        ...
        var record = ['sip','dip','protocol','spd_id','source_country','dest_country'];

        var _id = field.id;

        var _text = (_val==='sip')?__zen('src'):(_val==='dip')?__zen('dest'):(_val==='protocol')?__zen('service'):(_val==='spd_id')?__zen('sec_policy'):(_val==='source_country')?__zen('source_country'):__zen('dest_country');
        ...
        for(var i=0; i<_com.length; i++){
            ...
            if(me.ud_mode !== ''){
                ...
                if(a.indexOf('spd_id') !== -1){
                    _record.push({'name':__zen('sec_policy'),'val':'spd_id'});
                }
    ...
    onButtonClick: function(button, e, eOpts) {
        ...
        var a_null = ['ser_day_type','spc_time_type','st_time','dt_time','count','action','work','mode','s_purpose','order_sort','order_type'];
        ...
        var _store = Ext.data.StoreManager.lookup("store_tracker_fir_list");
        _store.removeAll();
    ...
    onTableExpandbody: function(rowNode, record, expandRow, eOpts) {
        ...
        for(var i in criteria.and){
            ...
            if(criteria.and[i].spd_id){ _and.push('spd_id'); }
        ...
        if(column.spd_id && _and.indexOf('spd_id') === -1){ criteria.and.push({'spd_id':record.data.spd_id}); }
    ...
    onGrid_listCellClick: function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
        ...
        if(Ext.getCmp("c_date").hidden === false){
            ...
            for(var i=0; i<step; i++){
                ...
                txt = (dep==="sip")?__zen('src'):(dep==="dip")?__zen('dest'):(dep==="protocol")?__zen('service'):(dep==="spd_id")?__zen('sec_policy'):(dep==="source_country")?__zen('source_country'):(dep==="dest_country")?__zen('dest_country'):"일별통계 전체 리스트";
    ...
    onPanelAfterRender: function(component, eOpts) {
        ...
        var store = Ext.data.StoreManager.lookup("store_tracker_fir_list");
    ...
    onNFW2_trackerDestroy: function(component, eOpts) {
        ...
        Ext.data.StoreManager.lookup("store_tracker_fir_list").removeAll();
    },
    ...
    get_tracker_search: function(col, day) {
        ...
        if(Ext.getCmp("mode").getValue() !== "0"){
            and.push({ 'direction': Number(Ext.getCmp('mode').getValue()) });
        }

        var action = Ext.getCmp("action").getValue();
        if(action === "1"){
            or.push({ 'action':1}, {'action':2});
        }else{
            and.push({ 'action': Number(action) });
        }
        ...
        for(var i=0; i<depth.step+1; i++){
            ...
            Ext.getCmp("grid_list").headerCt.move(_index,4+_h);
            _h++;
        }
        ...
        if(me.purpose === "1" && depth.step < h_step){
            group.push("time");
            store_total.getProxy().url = '/api/FtDBMgr/getFWTrackerTime';
            store.getProxy().url = '/api/FtDBMgr/getFWTrackerTime';
        }else{
            store_total.getProxy().url = '/api/FtDBMgr/getFWTracker';
            store.getProxy().url = '/api/FtDBMgr/getFWTracker';
        }
        showLoadMask();
        ...
        //store.getProxy().setExtraParam('locale',Ext.encode(__zen_locale));
    ...
    search: function() {
        ...
        var a_traffic = ["sip","dip","protocol","spd_id","source_country","dest_country"];
        var s_col = col.join(" ");
        ...
        for(var i=1; i<a_id.length; i++){
            ...
            var p_text = (p_dep==='sip'||p_dep==='dip')?Ext.getCmp("ip_"+a_id[i-1]).getValue():(p_dep==="protocol")?Ext.getCmp("protocol_"+a_id[i-1]).getValue():(p_dep==='spd_id')?Ext.getCmp("id_"+a_id[i-1]).getValue():(p_dep==='source_country')?Ext.getCmp("c_scode_"+a_id[i-1]).getValue():(p_dep==='dest_country')?Ext.getCmp("c_dcode_"+a_id[i-1]).getValue():'';
            ...
            if(m){
                var text = (dep==='sip'||dep==='dip')?Ext.getCmp("ip_"+a_id[i]).getValue():(dep==="protocol")?Ext.getCmp("protocol_"+a_id[i]).getValue():(dep==='spd_id')?Ext.getCmp("id_"+a_id[i]).getValue():(dep==='source_country')?Ext.getCmp("c_scode_"+a_id[i]).getValue():(dep==='dest_country')?Ext.getCmp("c_dcode_"+a_id[i]).getValue():'';
        ...
        if(a_id.length === 1){
            ...
            var text = (t_val==='sip'||t_val==='dip')?Ext.getCmp("ip_"+a_id[0]).getValue():(t_val==="protocol")?Ext.getCmp("protocol_"+a_id[0]).getValue():(t_val==='spd_id')?Ext.getCmp("id_"+a_id[0]).getValue():(t_val==='source_country')?Ext.getCmp("c_scode_"+a_id[0]).getValue():(t_val==='dest_country')?Ext.getCmp("c_dcode_"+a_id[0]).getValue():'';
