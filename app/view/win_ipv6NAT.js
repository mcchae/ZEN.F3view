...
Ext.define('NFW2.view.win_ipv6NAT', {
    extend: 'Ext.window.Window',
    alias: 'widget.win_ipv6nat',

    requires: [
        'NFW2.view.win_ipv6NATViewModel',
        'Ext.form.Panel',
        'Ext.grid.Panel',
                                    ...
                                    items: [
                                        {
                                            ...
                                            handler: function(view, rowIndex, colIndex, item, e, record, row) {
                                                ...
                                                me.err_nat.xdest = false; prt_errMsg("","fld_msg_fw");
                                            }
                                        }
                                    ]
                                }
                            ...
                            items: [
                                {
                                    ...
                                    handler: function(view, rowIndex, colIndex, item, e, record, row) {
                                        ...
                                        Ext.getCmp("win_ipv6NAT").get_obj_info(cid,otype,"grid_setobj",rowIndex,(record.data.xname===undefined)?record.data.name:record.data.xname,flag);
                                ...
                                },
                                {
                                    ...
                                    handler: function(view, rowIndex, colIndex, item, e, record, row) {
                                        var otype = record.data.otype;
                                        var loadmode = (otype==="port" || otype==="group")?"store_svc_obj":"store_ipv6_obj";
    ...
    onViewBeforeDrop2: function(node, data, overModel, dropPosition, dropHandlers, eOpts) {
        ...
        if(Ext.getCmp('type').getValue()==="TNAT"){
            if(otype === "env" || otype === "v6_group"){	return false;	}
        }
    ...
    onViewBeforeDrop21: function(node, data, overModel, dropPosition, dropHandlers, eOpts) {
        var otype = data.records[0].data.otype;
        if(otype === "port" || otype === "group" || otype === "v6_group"){	return false;	}
        if(Ext.getCmp('type').getValue()==="TNAT"){
    ...
    onViewItemAdd: function(records, index, node, eOpts) {
        console.log(records);
    },
    ...
    onViewDrop1: function(node, data, overModel, dropPosition, eOpts) {
        ...
        if(otype === "env"){me.err_nat.xsrc = false;  prt_errMsg("","fld_msg_fw"); return false;}
        if(otype === "v6"){
    ...
    onViewBeforeDrop11: function(node, data, overModel, dropPosition, dropHandlers, eOpts) {
        var otype = data.records[0].data.otype;
        if(otype !== "v6"){	return false;	}

        var _store = Ext.data.StoreManager.lookup("store_tmp_xdest");
        _store.removeAll();
    },

    onViewDrop2: function(node, data, overModel, dropPosition, eOpts) {
        var cid = data.records[0].data.cid;
        var otype = data.records[0].data.otype;
        if(otype !== "v6"){return false;}
        this.validNATType("xdest",cid);
    },
    ...
    onTextfieldKeydown: function(textfield, e, eOpts) {
        ...
        var _store = (Ext.getCmp('b_segment').getComponent(2).pressed===true)?"store_svc_obj":"store_ipv6_obj";
    ...
    onButtonClick22: function(button, e, eOpts) {
        ...
        if(Ext.getCmp('type').getValue()==="TNAT"){
            ...
            Ext.data.StoreManager.lookup('store_ipv6_obj').getProxy().setExtraParam("object_list",  Ext.encode(["env","ipv6"]));
            }
        }else{
            Ext.data.StoreManager.lookup('store_ipv6_obj').getProxy().setExtraParam("object_list",  Ext.encode(["env","ipv6","ipv6_group"]));
    ...
    onButtonClick21111: function(button, e, eOpts) {
        Ext.getCmp('grid_setobj').reconfigure("store_ipv6_obj");
        Ext.data.StoreManager.lookup('store_ipv6_obj').getProxy().setExtraParam("object_list",  Ext.encode(["env"]));
        Ext.data.StoreManager.lookup('store_ipv6_obj').load();
    },
    ...
    init_fw_nat_ipv6: function(edit, uid) {

        var me = this;
        me.err_nat = {xsrc:false,xdest:false};

        var _params = {
            basename : Ext.encode('firewall_nat_ipv6'),
    ...
    set_fw_nat_ipv6: function(edit, uid) {
        var me = this;


        if(me.err_nat.xsrc === true || me.err_nat.xdest=== true ){return false;}

        var __tmptotal = Ext.data.StoreManager.lookup('store_nat_ipv6_list').getTotalCount();
        ...
        function setalarmmsg(update){
            ...
            var _store = Ext.data.StoreManager.lookup('store_nat_ipv6_list');
            _store.load({
    ...
    setWinState_policy: function(btn) {
        ...
        if(btn==="no"){
                ...
                Ext.getCmp("fm_v6nat").getForm().reset();
    ...
    validNATType: function(id, cid) {
        var me = this;
        var title = (id==="xsrc")?__zen('xsrc'):__zen('xdest');
