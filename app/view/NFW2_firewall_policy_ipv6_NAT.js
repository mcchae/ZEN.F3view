...
Ext.define('NFW2.view.NFW2_firewall_policy_ipv6_NAT', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nfw2_firewall_policy_ipv6_nat',

    requires: [
        'NFW2.view.NFW2_firewall_policy_ipv6_NATViewModel',
    ...
    viewModel: {
        type: 'nfw2_firewall_policy_ipv6_nat'
    },
    cls: 'zen_body',
    ...
    on_btn_add: function(button, e, eOpts) {
        ...
        var total = (me.filter===true)?me.get_list():Ext.data.StoreManager.lookup('store_nat_ipv6_list').getTotalCount();

        if(total >= me.max){Ext.Msg.alert(__weguardia,ValidMaxCnt(me.max)); return false;}

        var win = Ext.create('NFW2.view.win_ipv6NAT',{
           modal : true,
           total : total
        });
    ...
    on_btn_del: function(button, e, eOpts) {
        ...
        var tbl_sel = Ext.getCmp("tbl_nat_ipv6").getSelectionModel().getSelection();
    ...
    onButtonClick1: function(button, e, eOpts) {
        ...
        _store.getProxy().setExtraParam("basename",  Ext.encode("firewall_nat_ipv6"));
    ...
    onTbl_spd_ipv4CellClick: function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
        ...
        if(cellIndex !== 0){
            ...
            if(cellIndex === 2){
                ...
                var _total = Ext.data.StoreManager.lookup('store_nat_ipv6_list').getTotalCount();

                var win = Ext.create('NFW2.view.win_v6nat_move',{
                    num : record.data['@num'],
                    uid : record.data['@uid'],
                    total : _total,
                    modal : true
            ...
            if(!(cellIndex === 5 ||cellIndex === 6 || cellIndex === 7 || cellIndex === 9 ||cellIndex === 10 || cellIndex === 11 || cellIndex === 12 )){	return false;	}


            var grid = Ext.getCmp("tbl_nat_ipv6");
            var store = grid.getStore();
            var expander = grid.findPlugin('rowexpander');

            if(cellIndex === 12){ //lasthit
            ...
            switch (cellIndex){
                ...
                case 9 : smode = 'xsrc'; break;
                case 10 : smode = 'xdest'; break;
                case 11 : smode = 'xservice'; break;
            }
            ...
            //var store = Ext.data.StoreManager.lookup("store_nat_ipv4_list");
    ...
    onNFW2_firewall_filtering_ipv4FilteringAfterRender: function(component, eOpts) {
        var me = this;
        /*request_helper.xmlrpc_call_JsonP('ftuctrl','getFileContent', { filename : Ext.encode('/proc/ferret/datasheet/nat_rule_line')},
             function(response){
                 me.max = response[0];
             }
        );*/
    ...
    export_rule: function(id, name) {
        ...
        var fileName = Ext.Date.format(currentDate, 'Ymd')+"_"+zeropad(currentDate.getHours())+zeropad(currentDate.getMinutes())+zeropad(currentDate.getSeconds())+"_"+name+"_IPv6NAT."+id;
        var path = '/mnt/ramdisk/do/F3view/nodeapps/deploy/public/javascripts/attachments/';
