...
Ext.define('NFW2.view.NFW2_firewall_policy_NAT', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nfw2_firewall_policy_nat',

    requires: [
        'NFW2.view.NFW2_firewall_policy_NATViewModel',
    ...
    on_btn_add: function(button, e, eOpts) {
        ...
        var total = (me.filter===true)?me.get_list():Ext.data.StoreManager.lookup('store_nat_ipv4_list').getTotalCount();

        if(total >= me.max){Ext.Msg.alert(__weguardia,ValidMaxCnt(me.max)); return false;}

        var win = Ext.create('NFW2.view.win_ipv4NAT',{
            modal : true,
            total : total
        });
    ...
    on_btn_del: function(button, e, eOpts) {
        ...
        var tbl_sel = Ext.getCmp("tbl_nat_ipv4").getSelectionModel().getSelection();
    ...
    onButtonClick1: function(button, e, eOpts) {
        ...
        _store.getProxy().setExtraParam("basename",  Ext.encode("firewall_nat_ipv4"));
    ...
    onTbl_spd_ipv4CellClick: function(tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
        ...
        if(cellIndex !== 0){
            ...
            if(cellIndex === 2){
                ...
                var _total = Ext.data.StoreManager.lookup('store_nat_ipv4_list').getTotalCount();

                var win = Ext.create('NFW2.view.win_nat_move',{
                    num : record.data['@num'],
                    uid : record.data['@uid'],
                    total : _total,
                    modal : true
            ...
            var grid = Ext.getCmp("tbl_nat_ipv4");
    ...
    export_rule: function(id, name) {
        ...
        var fileName = Ext.Date.format(currentDate, 'Ymd')+"_"+zeropad(currentDate.getHours())+zeropad(currentDate.getMinutes())+zeropad(currentDate.getSeconds())+"_"+name+"_IPv4NAT."+id;
        var path = '/mnt/ramdisk/do/F3view/nodeapps/deploy/public/javascripts/attachments/';
